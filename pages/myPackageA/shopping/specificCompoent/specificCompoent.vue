    ...
    <u-popup v-model="show" mode="center" border-radius="10" width="80%">
        <view>
            <view style="min-height: 200px;" class="container-box">
                ...
                <view v-for="type in commodity.FORMAT_TYPE">
                    {{ type }}
                    <view class="flex-box" style="flex-wrap: wrap;margin: 0 -10px;">
                        <view v-for="(item,index) in specific[type]" @click="onClick(type,item.label)"
                              style="width: 33.3%;padding: 10px;">
                            <view :class="{specificcurrbox:form[type] == item.label}"
